...
void interrupt(){
     if(intcon.intf==1){ 	// INT0 kesintisi meydana gelirse
          portd.b0 = ~portd.b0; // toggle pin D0 (toggle komutu lojik deðerini deðiþtirir 1'se 0 yapar, 0'sa 1 yapar.
          						// Örneðin; bir led yanýyorsa kapatýr veyahut yanmýyorsa yakar.
          for(i = 0; i < 100; i++){
               porte = i;
               delay_ms(300);
          }
          intcon.intf=0; 	//reset INT0 flag mutlka sýfýrlamalýyýz
     }
}
void main(){
